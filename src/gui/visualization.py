...
def _finalize_fig_to_texture(
    fig, texture_tag: str, parent_tag: str, width: int = 860, height: int = 320
):
    buf = io.BytesIO()
...
def plot_quick_distribution(
    df: pd.DataFrame, numeric_col: str, texture_tag: str, parent_tag: str
):
...
def plot_generic(
    df: pd.DataFrame, column: str, plot_type: str, texture_tag: str, parent_tag: str
):
    plt.style.use("dark_background")
    ...
    if plot_type == "Histogram":
        ...
        bp = ax.boxplot(
            df[column].dropna().values, vert=True, labels=[column], patch_artist=True
        )
        ...
        if pd.api.types.is_numeric_dtype(data_clean):
            parts = ax.violinplot(
                [data_clean.values], positions=[1], showmeans=True, showmedians=True
            )
            ...
            ax.text(
                0.5,
                0.5,
                "Violin plot requires numeric data",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
        ...
        data_clean = df[column].dropna()
        if pd.api.types.is_numeric_dtype(data_clean):
            data_clean.plot.density(
                ax=ax, alpha=0.8, color=DARK_COLORS["warning"], linewidth=2
            )
        else:
            ax.text(
                0.5,
                0.5,
                "Density plot requires numeric data",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
        ...
        if pd.api.types.is_numeric_dtype(data_clean):
            ...
            ax.text(
                0.5,
                0.5,
                "Q-Q plot requires numeric data",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
        ...
        if pd.api.types.is_numeric_dtype(data_clean):
            data_clean.plot.density(
                ax=ax, alpha=0.7, color=DARK_COLORS["light"], linewidth=2
            )
            ax.fill_between(
            ...
            ax.text(
                0.5,
                0.5,
                "Ridge plot requires numeric data",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
...
# --- Add: combinations add-on helpers (merged) ---


def plot_bar_topk(
    df: pd.DataFrame, col: str, k: int = 20, parent_tag: str = "plot_canvas"
):
    plt.style.use("dark_background")
...
def plot_heatmap_crosstab(
    df: pd.DataFrame,
    col_a: str,
    col_b: str,
    metric: str = "lift",
    max_levels: int = 30,
    parent_tag: str = "plot_canvas",
):
    plt.style.use("dark_background")
    fig, ax = plt.subplots(figsize=(10, 6), facecolor="#1a1a1a")
    ax.set_facecolor("#1a1a1a")
    a = df[col_a].astype("object")
    b = df[col_b].astype("object")
    top_a = a.value_counts().head(max_levels).index
    top_b = b.value_counts().head(max_levels).index
    a = a.where(a.isin(top_a))
    b = b.where(b.isin(top_b))
    ct = pd.crosstab(a, b).fillna(0)
    if ct.size == 0:
        ax.text(0.5, 0.5, "No data", ha="center", va="center")
        _finalize_fig_to_texture(fig, "hist_tex", parent_tag)
        return
    if metric == "lift":
        total = ct.values.sum()
    ...
    ax.set_xticklabels(
        [str(x) for x in ct.columns], rotation=45, ha="right", fontsize=9
    )
...
def plot_scatter_corr(
    df: pd.DataFrame, x: str, y: str, log: bool = False, parent_tag: str = "plot_canvas"
):
    plt.style.use("dark_background")
    fig, ax = plt.subplots(figsize=(10, 6), facecolor="#1a1a1a")
    ax.set_facecolor("#1a1a1a")
...
def plot_box_group(
    df: pd.DataFrame, cat: str, num: str, parent_tag: str = "plot_canvas"
):
    plt.style.use("dark_background")
    fig, ax = plt.subplots(figsize=(10, 6), facecolor="#1a1a1a")
    ax.set_facecolor("#1a1a1a")
...
def plot_corr_heatmap(
    ...
    ax.set_xticklabels(sel, rotation=45, ha="right", fontsize=9)
    ax.set_yticks(range(len(sel)))
    ax.set_yticklabels(sel, fontsize=9)
...
def plot_regression_with_ci(
    df: pd.DataFrame, x_col: str, y_col: str, parent_tag: str = "plot_canvas"
):
    ...
    if len(clean_data) < 3:
        ...
        ax.text(
            0.5,
            0.5,
            "Insufficient numeric data for regression",
            ha="center",
            va="center",
            color="#CCCCCC",
        )
        ...
        ax.text(
            ...
            bbox=dict(
                boxstyle="round,pad=0.3", facecolor=DARK_COLORS["accent"], alpha=0.7
            ),
            color="white",
            fontweight="bold",
        )
...
def plot_distribution_comparison(
    df: pd.DataFrame, col: str, group_col: str = None, parent_tag: str = "plot_canvas"
):
...
def plot_pair_correlation(
    ...
    if numeric_df.shape[1] < 2:
        ...
        ax.text(
            0.5,
            0.5,
            "Need at least 2 numeric columns for pair plot",
            ha="center",
            va="center",
        )
        _finalize_fig_to_texture(fig, "hist_tex", parent_tag, width=860, height=420)
        ...
        for j, col2 in enumerate(numeric_df.columns):
            ...
            if i == j:
                ...
                if len(clean_data) > 1:
                    ...
                    ax.text(
                        0.05, 0.95, f"r={corr:.2f}", transform=ax.transAxes, fontsize=8
                    )
            else:
...
def plot_time_series_decomposition(
    df: pd.DataFrame, date_col: str, value_col: str, parent_tag: str = "plot_canvas"
):
    ...
    try:
        ...
        if len(ts_data) < 3:
            axes[0, 0].text(
                0.5, 0.5, "Insufficient time series data", ha="center", va="center"
            )
        ...
        if len(ts_data) > 20:
            ...
            if len(ts_data) > 5000:
                axes[1, 1].text(
                    0.5,
                    0.5,
                    "Autocorrelation skipped for large time series",
                    ha="center",
                    va="center",
                    fontsize=9,
                )
...
# --- Advanced Visualization Functions (user-requested features) ---
    ...
    try:
        ...
        if len(clean_data) < 10:
            axes[0].text(
                0.5,
                0.5,
                "Insufficient data for scalar field",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
            _finalize_fig_to_texture(
                fig, "hist_tex", parent_tag, width=1120, height=420
            )
            return
        ...
        axes[0].scatter(
            x_vals, y_vals, c=z_vals, s=20, edgecolor="white", alpha=0.6, cmap="viridis"
        )
        ...
        axes[1].scatter(
            x_vals, y_vals, c=z_vals, s=30, edgecolor="white", alpha=0.8, cmap="viridis"
        )
    ...
    try:
        ...
        if len(clean_data) < 5:
            ax.text(
                0.5,
                0.5,
                "Insufficient data for gradient field",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
        ...
        ax.text(
            0.5,
            0.5,
            f"Error creating gradient field:\n{str(e)}",
            ha="center",
            va="center",
            color="#FF6B6B",
        )
    ...
    try:
        ...
        if len(clean_data) < 3:
            axes[0].text(
                0.5,
                0.5,
                "Insufficient data for enhanced scatter",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
        ...
        if size_col and size_col in clean_data.columns:
            sizes = (clean_data[size_col] - clean_data[size_col].min()) / (
                clean_data[size_col].max() - clean_data[size_col].min()
            )
        ...
        axes[0].text(
            0.5,
            0.5,
            f"Error creating enhanced scatter:\n{str(e)}",
            ha="center",
            va="center",
            color="#FF6B6B",
        )
    ...
    try:
        ...
        if len(clean_data) < 10:
            ax.text2D(
                0.5,
                0.5,
                "Insufficient data for 3D surface",
                ha="center",
                va="center",
                color="#CCCCCC",
            )
...
def plot_categorical_overview(
    df: pd.DataFrame, col: str, parent_tag: str = "plot_canvas"
):
