...
def build_left_panel(state: AppState):
    with dpg.child_window(tag=TAG_LEFT, width=300, autosize_y=True, border=True):
        with dpg.group(horizontal=True):
            dpg.add_text("CSV")
            dpg.add_text("ANALYZER", color=(65, 105, 225))
        dpg.add_separator()
        dpg.add_button(
            label="Load CSV File",
            width=-1,
            height=45,
            callback=lambda: open_csv_dialog(state),
        )
        ...
        with dpg.collapsing_header(label="Data Filters", default_open=True):
            ...
            dpg.add_radio_button(
                ...
                horizontal=False,
                tag="filter_condition",
            )
            dpg.add_input_text(label="Value", tag="filter_value")
            dpg.add_button(
                label="Apply Filter", width=-1, callback=lambda: on_apply_filter(state)
            )
            dpg.add_button(
                label="Reset", width=-1, callback=lambda: on_reset_filters(state)
            )
        ...
        with dpg.collapsing_header(label="Statistics", default_open=True):
            ...
            with dpg.table(header_row=False):
                ...
                dpg.add_table_column()
                with dpg.table_row():
                    dpg.add_text("Total Rows:")
                    dpg.add_text("0", tag="row_count")
                with dpg.table_row():
                    dpg.add_text("Columns:")
                    dpg.add_text("0", tag="col_count")
                with dpg.table_row():
                    dpg.add_text("Memory:")
                    dpg.add_text("0 MB", tag="memory_usage")
        ...
        with dpg.collapsing_header(label="Preview Controls", default_open=True):
            dpg.add_slider_int(
                ...
                min_value=50,
                max_value=1000,
                callback=lambda s, a: on_page_size_change(state),
            )
            with dpg.group(horizontal=True):
                dpg.add_button(
                    label="Prev Page", callback=lambda: on_change_page(state, -1)
                )
                dpg.add_spacer(width=8)
                dpg.add_button(
                    label="Next Page", callback=lambda: on_change_page(state, +1)
                )
    ...
    with dpg.child_window(tag=TAG_RIGHT, width=880, autosize_y=True):
        with dpg.tab_bar():
            with dpg.tab(label="Analysis"):
                with dpg.group(horizontal=True):
                    dpg.add_combo(
                        ["No columns loaded"], width=200, tag="analysis_column"
                    )
                    dpg.add_button(
                        label="Analyze Column",
                        callback=lambda: on_analyze_column(state),
                    )
            ...
            with dpg.tab(label="Visualization"):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_button(
                        label="Generate Plot", callback=lambda: on_generate_plot(state)
                    )
            ...
            with dpg.tab(label="Advanced Plots"):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_combo(
                        ...
                        width=150,
                        tag="adv_column2",
                        label="Column 2 (optional)",
                    )
                    dpg.add_button(
                        label="Generate Advanced Plot",
                        callback=lambda: on_generate_advanced_plot(state),
                    )
                with dpg.child_window(height=620, tag="advanced_plot_canvas"):
                    dpg.add_text(
                        "Select advanced plot options above", color=(130, 130, 140)
                    )
            ...
            if HAVE_COMB:
                with dpg.tab(label="Combinations"):
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(
                            label="Analyze",
                            callback=lambda: on_analyze_combinations(state),
                        )
                    with dpg.child_window(height=260, tag=TAG_COMB_SCROLL):
                        dpg.add_text(
                            "Run analysis to see combinations...", color=(130, 130, 140)
                        )
                    dpg.add_separator()
...
def on_file_selected(state: AppState, app_data):
    try:
        ...
        _safe_set_value(
            "memory_usage", f"{format_bytes(state.df.memory_usage(deep=True).sum())}"
        )
        ...
        if state.numeric_cols:
            try:
                set_progress(0.4)
                plot_quick_distribution(
                    state.df, state.numeric_cols[0], "hist_tex", TAG_PLOT_CANVAS
                )
            except Exception:
                pass
        set_progress(0.8)
...
def build_file_dialog(state: AppState):
    with dpg.file_dialog(
        ...
        show=False,
        callback=lambda s, a: on_file_selected(state, a),
        tag="file_dialog_id",
        width=700,
        height=400,
        modal=True,
...
def on_analyze_column(state: AppState):
    ...
    if not column:
        show_toast("컬럼을 선택하세요.", "warn")
        return
    dpg.delete_item(TAG_ANALYSIS_SCROLL, children_only=True)
    info = column_profile(state.df, column)
    dpg.add_text(
        f"Analysis for: {column}", color=(65, 105, 225), parent=TAG_ANALYSIS_SCROLL
    )
...
def on_generate_plot(state: AppState):
    ...
    if not column or not plot_type:
        show_toast("컬럼/플롯 타입을 선택하세요.", "warn")
        return
...
def on_generate_advanced_plot(state: AppState):
    ...
    if not plot_type:
        show_toast("고급 플롯 타입을 선택하세요.", "warn")
        return
    if not column1:
        show_toast("최소 하나의 컬럼을 선택하세요.", "warn")
        return
    ...
    try:
        if plot_type == "Regression Analysis":
            if not column2:
                show_toast("회귀 분석에는 두 개의 컬럼이 필요합니다.", "warn")
                return
            ...
            plot_distribution_comparison(
                state.df,
                column1,
                column2 if column2 != "No columns loaded" else None,
                "advanced_plot_canvas",
            )
            ...
            if len(numeric_cols) < 2:
                ...
                return
            plot_pair_correlation(
                state.df, numeric_cols, parent_tag="advanced_plot_canvas"
            )
            ...
            if not column2:
                ...
                return
            plot_time_series_decomposition(
                state.df, column1, column2, "advanced_plot_canvas"
            )
...
def on_apply_filter(state: AppState):
    ...
    if not col:
        ...
        return
    if not cond:
        show_toast("조건을 선택하세요.", "warn")
        return
    if val is None or str(val).strip() == "":
        show_toast("값을 입력하세요.", "warn")
        return
    try:
        ...
        _safe_set_value(
            "status",
            f"Filter applied: {len(state.filtered_df) if state.filtered_df is not None else len(state.df)} rows",
        )
...
def on_change_page(state: AppState, delta: int):
    if state.df is None:
        return
    total_rows = (
        len(state.filtered_df) if state.filtered_df is not None else len(state.df)
    )
...
def on_analyze_combinations(state: AppState):
    ...
    try:
        ...
        LAST_REPORT = report
        dpg.add_text(
            f"Rows: {report['meta']['rows']} | Num cols: {len(report['meta']['num_cols'])} | Cat cols: {len(report['meta']['cat_cols'])}",
            parent=TAG_COMB_SCROLL,
        )
        ...
        for entry in report.get("numnum", [])[:3]:
            a, b = entry["cols"]
            strength = max(
                abs(entry.get("pearson") or 0.0), abs(entry.get("spearman") or 0.0)
            )
            dpg.add_text(
                f"Num×Num: {a} vs {b} (|r|max={strength:.2f})", parent=TAG_COMB_SCROLL
            )
        for entry in report.get("catnum", [])[:3]:
            c, n = entry["cols"]
            dpg.add_text(
                f"Cat×Num: {c} -> {n} (eta²={entry['eta2']:.2f})",
                parent=TAG_COMB_SCROLL,
            )
        recs = suggest_plots(report)
        if not recs:
            dpg.add_text(
                "No recommendations.", color=(130, 130, 140), parent=TAG_RECO_GROUP
            )
        else:
            for rec in recs:
                t = rec["type"]
                ...
                def _cb(kind=t, cols=cols):
                    _do_plot_from_reco(state, kind, cols)
