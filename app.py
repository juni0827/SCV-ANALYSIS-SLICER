...
class CSVAnalyzerApp:
    def __init__(self, root):
        ...
        self.plotter = Plotter(self.is_dark_mode)
        self.setup_themes()
        self.setup_styles()
        self.setup_ui()
    ...
    def toggle_dark_mode(self):
        ...
        self.current_theme = self.themes['dark'] if self.is_dark_mode else self.themes['light']
        self.plotter.update_theme(self.is_dark_mode)
        ...
        # 시각화가 존재하면 다시 그리기 (테마 적용)
        if hasattr(self, 'viz_frame') and self.viz_frame.winfo_children():
            self.root.after(300, self.create_visualization)
...
분석 결과는 아래에 표시됩니다.
    ...
    def build_analysis_tab(self):
        ...
        self.column_combo = ttk.Combobox(control_frame, textvariable=self.column_var,
                                        values=["No columns loaded"], width=20)
        self.column_combo.pack(side='left', padx=(0, 10))

        # 시각화 유형 선택
        self.viz_type_var = tk.StringVar(value="Auto (Smart)")
        self.viz_type_combo = ttk.Combobox(control_frame, textvariable=self.viz_type_var,
                                          values=[
                                              "Auto (Smart)", 
                                              "Histogram", "Box Plot", "Scatter Plot", "Line Plot", 
                                              "Bar Chart", "Pie Chart", "Donut Chart",
                                              "3D Scatter", "Violin Plot", "Heatmap", "Hexbin Plot"
                                          ], width=15)
        self.viz_type_combo.pack(side='left', padx=(0, 10))

        # 분석 버튼
        analyze_btn = self.create_styled_button(control_frame, "Analyze", 
                                               self.analyze_column, '#3498DB')
        analyze_btn.pack(side='left', padx=(0, 10))

        # 시각화 버튼
        visualize_btn = self.create_styled_button(control_frame, "Visualize", 
    ...
    def create_visualization(self):
        ...
        try:
            ...
            self.plotter.plot(viz_type, data, column, self.viz_frame)
